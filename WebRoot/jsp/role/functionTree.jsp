...
<body>
	...
	<ul class='simpleTree'>
		...
		<!-- 第一层 start -->
		<ul>
		<c:forEach items="${functionList }" var="fun1">
		
			<li id='${fun1.id }'><input type="checkbox" <c:if test="${fun1.flag }">checked</c:if> name="functionIds" value="${fun1.id }"><span>${fun1.name }</span>
				<!-- 第二层 start -->
				<ul>
				<c:forEach items="${fun1.childMenus }" var="fun2">
					<li id='${fun2.id }'><input type="checkbox" <c:if test="${fun2.flag }">checked</c:if> name="functionIds" value="${fun2.id }"><span>${fun2.name }</span>
					<!-- 第三层 start -->
					<c:if test="${fn:length(fun2.childMenus)>0 }">
					<ul>
					<c:forEach items="${fun2.childMenus }" var="fun3">
						<li id='${fun3.id }'><input type="checkbox" <c:if test="${fun3.flag }">checked</c:if> name="functionIds" value="${fun3.id }"><span>${fun3.name }</span></li>
					</c:forEach>	
					</ul>
					</c:if>
					<!-- 第三层 end -->
